...
def yandex_get_info(geoposition: str) -> dict:
    ...
    data_yandex_icon = soup_yandex.find_all("div", class_="style_weatherIcon__OE4YL AppShortForecastDay_icon__rlqHK")
    ...
    for day in range(len(data_yandex_day)):
        if data_yandex_date[day].text == "Today":
            ...
            info_by_yandex[1] = [data_yandex_temp[day * 2], data_yandex_temp[day * 2 + 1], data_yandex_day[day].text,
                                 int(data_yandex_icon[day].get("style").lstrip("--icon:"))]
        else:
